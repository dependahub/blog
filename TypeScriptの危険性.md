...
これが backend にカーソルを合わせると出てくるヒントですが、そもそもキーが動的ですが、```type Backend``` は ```@aws-amplify/backend``` から取り出せますが、その型引数はどこからどう引っ張れば取り出せるのか謎。

こう言う場合、createApi() 関数はこう書くしかありません。（CDKの内容なんて ```Backend<any>``` ではとても生成できません）
